...
public partial class BoardView3D : global::Godot.Control
{
    ...
    /// <summary>Neutral ground distinguishes hidden food from empty brown soil.</summary>
    public static readonly Color OrganismsGroundColor = new("c1c1c1");
    ...
    /// <summary>Uses the original viewer's full RGB DNA encoding; the completed snapshot already supplies it.</summary>
    public static Color ColonyColor(ViewCell cell, string? highlightedPattern = null)
    {
        uint rgb = unchecked((uint)cell.ColorArgb);
        Color color = new(((rgb >> 16) & 255) / 255f, ((rgb >> 8) & 255) / 255f, (rgb & 255) / 255f, 1);
    ...
    private void UploadFrame()
    {
        ...
        foreach (ViewCell cell in frame.Cells)
        {
            ...
            Color ground = layer == BoardLayer.Organisms ? OrganismsGroundColor : FoodColor(cell.Food);
            ...
            if (cell.OrganismId.HasValue && layer != BoardLayer.Food)
                WriteInstance(organismBuffer, colonyCount++, cell.X + .5f, .024f, cell.Y + .5f,
                    DetailLevel == 1 ? .88f : .92f, ColonyColor(cell), seed,
                    highlightedPattern != null && cell.PatternKey != highlightedPattern ? .36f : 1);
        }
