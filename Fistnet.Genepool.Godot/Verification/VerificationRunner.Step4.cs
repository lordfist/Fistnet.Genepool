...
public partial class VerificationRunner
{
    private async Task DepthPassiveChecks()
    {
        ...
        foreach (int mode in new[] { 0, 1 })
        {
            ...
            foreach (int level in new[] { 1, 2, 3, 4 })
            {
                ...
                // A perspective footprint is a polygon, not all cells in its bounding rectangle.
                // PerspectiveChecks independently checks this predicate against native camera ray clipping.
                int expectedFood = fixture.Cells.Count(cell => board.IsCellVisible(cell.X, cell.Y));
                int expectedOrganisms = fixture.Cells.Count(cell => cell.OrganismId.HasValue && board.IsCellVisible(cell.X, cell.Y));
            ...
            Check($"depth extreme pan remains finite mode {mode}", board.VisibleArea.Position.IsFinite() && board.VisibleArea.Size.IsFinite());
            // Screen-space diagonal dragging does not mean ground-space north-west once the camera has yaw.
            board.CenterOn(0, 0); await Frames(2);
    ...
    private async Task DepthMinimapChecks()
    {
        ...
        Check("minimap outline follows native perspective ground footprint", SamePolygon(footprint, main.Minimap.ViewFootprint, .015f));
        board.CenterOn(50, 50);
    }
    ...
    private async Task DepthPixelChecks()
    {
        ...
        Color dry, lush, plain, blackGround;
        using (var image = board.BoardViewport.GetTexture().GetImage())
        {
            ...
            blackGround = AveragePatch(image, board.CellCenter(52, 50), 4);
        ...
        using (var image = board.BoardViewport.GetTexture().GetImage())
        {
            ...
            Check("black DNA retains a black body visible against its actual ground",
                Math.Max(darkDna.R, Math.Max(darkDna.G, darkDna.B)) < .15f && ColorDistance(darkDna, blackGround) > .10f,
                $"body={darkDna}; bare ground={blackGround}");
        }
