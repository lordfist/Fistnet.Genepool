...
public partial class MinimapView : Godot.Control
{
    ...
    {
        TooltipText = "The whole world. The outline shows your visible ground area. Click to move the camera.";
    ...
    private void Upload()
    {
        ...
        foreach (ViewCell cell in frame.Cells)
        {
            ...
            Color color = cell.OrganismId.HasValue && layer != BoardLayer.Food ? BoardView3D.ColonyColor(cell) :
                layer == BoardLayer.Organisms ? BoardView3D.OrganismsGroundColor : BoardView3D.FoodColor(cell.Food);
    ...
    public override void _Draw()
    {
        ...
        if (texture != null && frame != null)
        {
            ...
            if (outline.Count > 1)
            {
                ...
                points[^1] = points[0];
                DrawPolyline(points, layer == BoardLayer.Organisms ? new Color("35434d") : new Color("ffeda5"), 1.5f, true);
            }
            if (selected is { } cell)
            {
                Vector2 point = Map.Position + new Vector2(cell.X + .5f, cell.Y + .5f) * scale;
                if (layer == BoardLayer.Organisms) DrawCircle(point, 3, new Color("35434d"));
