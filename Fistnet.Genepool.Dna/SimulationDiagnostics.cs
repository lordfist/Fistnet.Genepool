    ...
    public sealed class DiagnosticSession
    {
        ...
        public void EffectApplied(Organism recipient, IDnaEffect effect, DiagnosticState before)
        {
            ...
            if (effect.Effect == EffectTypes.HealthChange)
            {
                ...
                long expected = (long)before.Health + change;
                if (recipient.HealthRule == HealthPolicy.Capped && expected > Common.Policy.MaximumHealth
                    && after.Health == Common.Policy.MaximumHealth) Count("health.capped");
                else if (expected != after.Health) AddContributor(recipient, Contributor.ArithmeticWrap);
            ...
            else if (effect.Effect == EffectTypes.FoodChange)
            {
                ...
                int uptake = Math.Max(0, after.TakenFood - before.TakenFood);
                ...
                if (requested > 0 && after.TakenFood < before.TakenFood) Count("food.taken_overwrite");
            }
