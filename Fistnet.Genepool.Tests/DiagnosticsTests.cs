...
internal static class DiagnosticsTests
{
    public static IEnumerable<TestCase> Cases()
    {
        ...
        yield return new("diagnostics: source-cell food debits stay correct across movement and death", "diagnostics", PendingFood);
    ...
    {
        EmptyReference();
        var organism = new FixtureOrganism(); organism.SetState(food: 0);
        organism.SetGenes((owner, index) => { var gene = new CreateFoodDnaElement(owner, index); Check.Property(gene, "Target", TargetTypes.Self); return gene; });
        var square = Board.BoardElement[5, 5]; square.AddOccupant(organism);
        var report = Observe(() =>
        {
            organism.Activate(); organism.SetAvailableFood(square.FoodRemaining);
            void Gather()
            {
                var candidates = new[] { new ActionCandidate(organism.DnaSequence[0], organism) };
                var decision = organism.PrepareSeason(candidates);
                organism.ResolveDecision(decision, (recipient, requested) => square.GatherFood(recipient, requested), null);
                organism.FinishSeason(decision, true, true);
            }
            Gather(); square.IncreaseFood(2); organism.SetAvailableFood(square.FoodRemaining); Gather();
            Check.Equal(5, organism.FoodBalance); Check.Equal(5, organism.TakenFood);
            Check.Equal((byte)0, square.FoodRemaining);
            new ExecuteOgranismSetup().Execute(square);
            Check.Equal((byte)0, square.FoodRemaining, "Setup debited uptake again");
        });
        Check.Equal(2L, report.Totals["actions.attempted.GenerateFood"]);
        Check.Equal(2L, report.Totals["effects.applied.FoodChange"]);
        Check.Equal(5L, report.Totals["food.actual_uptake"]);
        Check.Equal(0L, report.Totals.GetValueOrDefault("food.taken_overwrite"));
        Check.Equal(5L, report.Totals["food.debit_applied"]);
    }
    ...
    private static void BirthAndMovement()
    {
        ...
        // Only one birth gap, so diagnostic direction has a defined expected value.
        foreach (TargetTypes direction in Enum.GetValues<TargetTypes>())
            if (direction != TargetTypes.TopLeft && direction != TargetTypes.Self)
                Board.BoardElement[10, 10].GetNeightbor(direction).AddOccupant(new Organism());
        var report = Observe(() =>
        {
            Check.True(Board.BoardElement[10, 10].TryPlaceChild(other), "Birth failed");
            Check.True(Board.BoardElement[9, 9].IsOccupied, "Only birth gap not used");
            Check.True(parent.HasChild && parent.Child == null, "Actual birth must not retain pending child");
            Board.BoardElement[11, 10].RemoveOccupant();
            Check.True(Board.BoardElement[10, 10].TryMove(TargetTypes.MiddleRight), "Move failed");
            Check.True(!Board.BoardElement[11, 10].TryMove(TargetTypes.Self), "Self movement accepted");
            Check.True(ReferenceEquals(parent, Board.BoardElement[11, 10].Occupant), "Move lost identity");
        });
        ...
        Check.True(report.Events.Any(e => e.Kind == "birth.created"), "Missing construction event");
    }
    ...
    private static void WorkloadFixture()
    {
        ...
        try
        {
            ...
            ScenarioRunner.ValidateWorld(allowSyntheticReserves: true);
    ...
    private static void PendingFood()
    {
        ...
        var report = Observe(() =>
        {
            foreach (var organism in new[] { moving, dying })
            {
                ...
                Board.BoardElement[ReferenceEquals(organism, moving) ? 20 : 30, ReferenceEquals(organism, moving) ? 20 : 30].GatherFood(organism, 2);
            ...
            Check.Equal((byte)1, Board.BoardElement[20, 20].FoodRemaining, "Original cell was not debited");
            Check.Equal((byte)3, Board.BoardElement[21, 20].FoodRemaining, "Uptake incorrectly followed the moving occupant");
            ...
            Check.Equal((byte)8, Board.BoardElement[30, 30].FoodRemaining, "Death return must not duplicate already withdrawn food");
        });
        Check.Equal(4L, report.Totals["food.uptake_transferred"]);
        Check.Equal(4L, report.Totals["food.debit_applied"]);
        Check.Equal(0L, report.Totals.GetValueOrDefault("food.pending_debit_at_death"));
