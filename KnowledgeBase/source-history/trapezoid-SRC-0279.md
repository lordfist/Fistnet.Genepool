...
The default viewer uses a Godot 3D viewport with an angled perspective camera and modest diagonal orientation. The near edge is wider, the far edge narrower, and the grid recedes into the distance. Choose **Habitat · top-down** for a direct overhead orthographic view, or **Original 2D** for the earlier renderer. All three observe the same running world; switching views does not restart it. The Windows Forms application remains a separate startup project.

The new terrain runs from brown dirt through olive to green using the existing food quantity. Close views reveal small grass fans. Organisms look like flat, irregular mould colonies with fuzzy edges. Their decorative shape stays stable as they move. Colors provide contrast and pattern grouping; the finite palette is not a unique DNA identifier or species system. These materials are generated on the GPU and require no downloaded artwork or new dependency.
...
For Step 4 use `--mode step4-headless`, `--mode step4`, and `--mode step4-benchmark` respectively. The new benchmark includes 0/1,000/10,000-organism overview fixtures plus a dense close view with complete outcome batches. These opt-in scenes close their own window and simulation worker when finished. The latest perspective correction, current checks and actual rendered screenshots are linked in `KnowledgeBase/R03_STEP4_PERSPECTIVE_RESULT.md`; the initial Step 4 result remains in `KnowledgeBase/IMPROVEMENT_RESULT_R03_STEP4.md` as dated evidence.
...
These are the earlier 2D measurements, retained in `KnowledgeBase/IMPROVEMENT_RESULT_R03_2D.md`; that practical viewer was accepted by the owner. Step 4's latest correction and pending owner review are recorded separately in `KnowledgeBase/R03_STEP4_PERSPECTIVE_RESULT.md`. Step 5 graphics polish and standalone export remain later work.
