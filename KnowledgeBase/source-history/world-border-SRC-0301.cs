...
public partial class VerificationRunner
{
    ...
    private async Task Step5RgbPixelChecks()
    {
        ...
        foreach (var backdrop in new[] { ("grey", 1, 0), ("brown", 0, 0), ("olive", 0, 5), ("green", 0, 10) })
        {
            ...
            for (int level = 1; level <= 3; level++)
            {
                ...
                string detail = string.Join("; ", samples.OrderBy(sample => sample.ContrastFraction).Take(5).Select(sample =>
                    $"{sample.Name}: contrast={sample.ContrastPixels}/{sample.CellPixels}, RGBerror={sample.CoreChannelError:0.000}"));
                Check($"full RGB bodies or edges remain visible on {backdrop.Item1} at detail {level}",
                    samples.All(sample => level == 1 ? sample.ContrastPixels >= 1 : sample.ContrastFraction >= .025), detail, false);
                if (level > 1)
                ...
                measurements.Add(new { kind = "step5_rgb_visibility", backdrop = backdrop.Item1, detailLevel = level, sampleCount = samples.Count,
                    ...
                    criteria = "RGB distance>=0.18 in at least1 World pixel or2.5% of cell at levels2/3; central body per-channel error<=0.12 at levels2/3. No saturation or restricted-palette criterion." });
            }
        }
    ...
    private sealed record RgbPixelSample(string Name, int CellPixels, int ContrastPixels, int CorePixels, double CoreChannelError)
    {
    ...
    private static RgbPixelSample MeasureRgbPixels(BoardView3D board, Vector2I cell, (string Name, int Rgb) sample, Image ground, Image organisms)
    {
        ...
        int pixels = 0, contrast = 0, corePixels = 0; Color core = new(0, 0, 0, 0);
        ...
        for (int x = left; x <= right; x++)
        {
            ...
            pixels++;
            Color current = organisms.GetPixel(x, y);
            if (ColorDistance(ground.GetPixel(x, y), current) >= .18f) contrast++;
        ...
        return new RgbPixelSample(sample.Name, pixels, contrast, corePixels, error);
    }
