...
public partial class MinimapView : Godot.Control
{
    ...
    {
        TooltipText = "The whole world. The white rectangle is your visible area. Click to move the camera.";
    ...
    public override void _Draw()
    {
        ...
        if (texture != null && frame != null)
        {
            ...
            Rect2 world = new(0, 0, frame.Width, frame.Height);
            Rect2 visible = area.Intersection(world);
            Vector2 scale = Map.Size / new Vector2(frame.Width, frame.Height);
            DrawRect(new Rect2(Map.Position + visible.Position * scale, visible.Size * scale), new Color("ffeda5"), false, 1.5f);
            if (selected is { } cell)
