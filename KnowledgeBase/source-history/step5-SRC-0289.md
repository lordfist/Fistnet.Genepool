...
For Step 4 use `--mode step4-headless`, `--mode step4`, and `--mode step4-benchmark` respectively. The new benchmark includes 0/1,000/10,000-organism overview fixtures plus a dense close view with complete outcome batches. These opt-in scenes close their own window and simulation worker when finished. The latest trapezoid and grey-ground refinement, current checks and actual rendered screenshots are linked in `KnowledgeBase/R03_STEP4_TRAPEZOID_RESULT.md`; earlier Step 4 results remain as dated evidence.
