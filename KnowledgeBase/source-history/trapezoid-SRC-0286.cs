...
public partial class VerificationRunner
{
    private async Task PerspectiveChecks()
    {
        ...
        foreach (var size in new[] { new Vector2I(1366, 768), new Vector2I(1920, 1080) })
        {
            ...
            Check("perspective front edge is visibly wider than back " + size, front > back * 1.12f,
                $"front={front:0.00}px; back={back:0.00}px; ratio={front / back:0.000}");
            float diagonal = Math.Abs(corners[1].Y - corners[0].Y) / back;
            Check("perspective board has a modest visible diagonal " + size, diagonal is > .08f and < .55f,
                $"back edge vertical component / length={diagonal:0.000}");
