...
/// <summary>Detached observation rendered as two visible-cell GPU batches on an orthographic XZ board.</summary>
public partial class BoardView3D : global::Godot.Control
{
    ...
    private Vector2 center = new(50, 50), dragStart, dragCenter;
    ...
    public float ProjectedCellSize => PixelsPerCell * VerticalFactor;
    ...
    public Rect2 VisibleArea => new(center - new Vector2(Size.X / PixelsPerCell, Size.Y / (PixelsPerCell * VerticalFactor)) / 2,
        new Vector2(Size.X / PixelsPerCell, Size.Y / (PixelsPerCell * VerticalFactor)));
    public Rect2I VisibleBounds
    {
        get
        {
            if (frame == null) return new Rect2I();
            Rect2 area = VisibleArea;
            int x = Math.Clamp((int)Math.Floor(area.Position.X), 0, frame.Width);
            int y = Math.Clamp((int)Math.Floor(area.Position.Y), 0, frame.Height);
            int endX = Math.Clamp((int)Math.Ceiling(area.End.X), x, frame.Width);
            int endY = Math.Clamp((int)Math.Ceiling(area.End.Y), y, frame.Height);
    ...
    private float VerticalFactor => angled ? .8f : 1;
    private float FitPixels => Math.Max(.001f, Math.Min(Math.Max(1, Size.X - 28) / (frame?.Width ?? 100),
        Math.Max(1, Size.Y - 28) / ((frame?.Height ?? 100) * VerticalFactor)));
    private float PixelsPerCell => FitPixels * zoom;
    ...
    public override void _Ready()
    {
        ...
        camera = new Camera3D
        {
            Name = "OrthographicBoardCamera", Projection = Camera3D.ProjectionType.Orthogonal,
    ...
    private void UploadFrame()
    {
        ...
        {
            if (!bounds.HasPoint(new Vector2I(cell.X, cell.Y))) continue;
    ...
    public void SetAngled(bool value)
    {
        ...
        angled = value;
        zoom = Math.Clamp(previous / (FitPixels * VerticalFactor), 1, 80);
    ...
    public void SetDetailLevel(int value)
    {
        ...
        zoom = Math.Clamp(target / (FitPixels * VerticalFactor), 1, 80);
        UpdateCamera(); ViewChanged?.Invoke();
    }
    ...
    public void ZoomAt(float factor, Vector2 localAnchor)
    {
        ...
        if (next == zoom) return;
        Vector2 before = WorldAt(localAnchor); zoom = next;
        center += before - WorldAt(localAnchor);
        UpdateCamera(); ViewChanged?.Invoke();
    }
    public void PanBy(Vector2 localDistance)
    {
        center -= new Vector2(localDistance.X / PixelsPerCell, localDistance.Y / (PixelsPerCell * VerticalFactor));
        UpdateCamera(); ViewChanged?.Invoke();
    }
    private Vector2 WorldAt(Vector2 local) => center + new Vector2((local.X - Size.X / 2) / PixelsPerCell,
        (local.Y - Size.Y / 2) / (PixelsPerCell * VerticalFactor));
    private Vector2 Project(Vector2 world) => Size / 2 + new Vector2((world.X - center.X) * PixelsPerCell,
        (world.Y - center.Y) * PixelsPerCell * VerticalFactor);
    ...
    public Vector2 CellCenter(int x, int y) => Project(new Vector2(x + .5f, y + .5f));
    public (int X, int Y)? CellAt(Vector2 localPosition)
    {
        ...
        Vector2 world = WorldAt(localPosition);
    ...
    private void UpdateCamera()
    {
        ...
        float halfX = Size.X / PixelsPerCell / 2, halfY = Size.Y / (PixelsPerCell * VerticalFactor) / 2;
        center.X = halfX >= width / 2f ? width / 2f : Math.Clamp(center.X, halfX - 10 / PixelsPerCell, width - halfX + 10 / PixelsPerCell);
        center.Y = halfY >= height / 2f ? height / 2f : Math.Clamp(center.Y, halfY - 10 / ProjectedCellSize, height - halfY + 10 / ProjectedCellSize);
        camera.Size = Math.Max(.01f, Size.Y / PixelsPerCell);
        Vector3 target = new(center.X, 0, center.Y);
        camera.Position = target + (angled ? new Vector3(0, 80, 60) : new Vector3(0, 100, 0));
        camera.LookAt(target, angled ? Vector3.Up : new Vector3(0, 0, -1));
    ...
    public override void _GuiInput(InputEvent input)
    {
        if (input is InputEventMouseButton button)
        {
            ...
            else if (button.ButtonIndex is MouseButton.Right or MouseButton.Middle)
            {
                ...
                if (dragging) { dragStart = button.Position; dragCenter = center; GrabFocus(); }
        ...
        {
            Vector2 distance = motion.Position - dragStart;
            center = dragCenter - new Vector2(distance.X / PixelsPerCell, distance.Y / ProjectedCellSize);
