...
namespace Fistnet.Genepool.Dna
{
    public class Organism : OrganismSnapshot
    {
        ...
        public void ExecuteEffectStack()
        {
            ...
            {
                DiagnosticState before = diagnostics == null ? default : new DiagnosticState(this);
                this.ExecuteSingleEffect(item);
                diagnostics?.EffectApplied(this, item, before);
            }
            this.Starved();
        ...
        private void Starved()
        {
            if (this.FoodBalance < 0)  // STARVING
            {
                ...
                this.FoodBalance = 0;
                diagnostics?.Starvation(this, before);
            }
        }
        ...
        private void BirthChild(Organism otherParent)
        {
            if (this._reproductionsInAge < Organism.MAX_REPRODUCTIONS_PER_AGE && this.FoodBalance > 0)
            {
                ...
                SimulationDiagnostics.Current?.ChildCreated(this, otherParent, this.Child);
            }
            this._reproductionsInAge++;
        }
        ...
        public void ReleaseChild()
        {
            if (this.Child != null)
            {
                using (SimulationDiagnostics.Current?.BeginSystemEffect(this, "birth-cost"))
        ...
        {
            byte chosenIndex = 0;
            IDnaElement chosen = this.Brain.ChooseOutput(null, out chosenIndex);
            var diagnostics = SimulationDiagnostics.Current;
            if (diagnostics != null)
            {
                diagnostics.Count("choices.target_probes");
                diagnostics.Event("choice.target_probe", this, -1, -1, diagnostics.CanSampleEvents
                    ? "Slot=" + chosenIndex + "; action=" + chosen.DnaType + "; target=" + chosen.Target : null);
            }
        ...
        public void ExecuteNextDnaSequence(Organism organismAffected)
        {
            ...
            if (this.Age >= Organism.MAX_AGE)
            {
                ...
                DiagnosticState before = diagnostics == null ? default : new DiagnosticState(this);
                this.Health = 0;
                diagnostics?.AgeLimit(this, before);
            }

            if (!this.IsDead && currentSequenceIndex < Organism.DNA_SEQUENCE_MAXLENGTH)
            {
                IDnaElement chosen = this.Brain.ChooseOutput(organismAffected, out chosenSequenceIndex);
                using (SimulationDiagnostics.Current?.BeginAction(this, chosen, organismAffected))
                    chosen.ExecuteDna(organismAffected);
