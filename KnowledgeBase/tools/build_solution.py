"""Local VS2022 build helper; disposable SDK/cache configuration, compact current results."""
import argparse
import collections
import datetime
import hashlib
...
def main():
    ...
    for project in ROOT.glob("Fistnet.Genepool.*"):
        if project.is_dir():
            for path in project.rglob("*"):
                if path.suffix in {".cs", ".csproj"} and not set(path.parts) & {"bin", "obj", ".vs"}:
    ...
    with tempfile.TemporaryDirectory(prefix=".build-", dir=KB) as temp:
        ...
        for key, folder in {"LOCALAPPDATA":"local", "APPDATA":"roaming", "TEMP":"tmp", "TMP":"tmp",
                            "DOTNET_CLI_HOME":"cli", "NUGET_PACKAGES":"packages", "NUGET_HTTP_CACHE_PATH":"http",
        ...
        commands = [
            [DOTNET, "restore", solution, "--configfile", str(config), "--disable-parallel", *common],
