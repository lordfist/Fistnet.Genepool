# Seed KnowledgeBase policy and reference

Generated by Other and its transient packaging helper, 2026-09-05. This is package guidance, not experiment evidence. The `CONSTITUTION.md` remains the general contract, with the owner label generalized to User and its Seed Analyzer identity and date retained. Higher-priority instructions and current owner boundaries control.

## Required operating policy

### Binding and authority

Installation supplies tools and empty derived working knowledge. It does not activate the Seed, assign an experiment, accept a model or architecture, establish domain facts or deployment, or authorize a trial. Package provenance is software metadata. The KnowledgeBase is support infrastructure, outside the original experiment corpus unless the owner explicitly makes it a subject of study.

The tools require `<root>/KnowledgeBase/tools`; their physical installation root must equal the initialized `core.json` root. The owner must explicitly assign that same exact physical directory as the experiment root and request a stage of work. Neither the installed path, cwd nor parent repository supplies that assignment. If a different root is chosen, install the untouched distribution there or obtain a bounded migration/rebinding assignment; moving an initialized core is not automatic rebinding. A different role also requires an explicit adaptation of the constitution.

The installed KnowledgeBase becomes working knowledge only when the owner identifies its exact root. Routine maintenance requires explicit authority for bundled-tool execution, intended retrieval/validation, registered source preparation, and transactions, using permitted experiment sources and writing derived knowledge only inside that KnowledgeBase. The owner may grant this at binding or later, narrow it, or keep it disabled. Installation or a request for analysis alone does not grant it.

When enabled, this is a narrow exception to the constitution's read-only, non-executing discovery rules. Original sources remain read-only and non-executing. It does not authorize tool/package changes, outside-root reads, external research, dependency installation, source repairs, other Actors, or experiment execution. Honor narrower current assignments and blind/private/excluded evidence boundaries. Do not repeat permission requests for effects already clearly authorized. Without the exception, continue authorized file inspection and conversational analysis without unauthorized tool execution or knowledge writes.

**Owner-authorized local adaptation, 2026-09-06:** User subsequently authorized the App solution-configuration repair, then instructed Seed Analyzer to adapt KB tools to this experiment, including source revisions backed by Git. These tools are working aids that may be changed within that authorization; the packaged implementation is not an immutable rule. Preserve KB integrity and honest provenance while removing unnecessary obstacles. Test material tool changes with disposable synthetic fixtures before canonical use. Git versions experiment sources, not this KnowledgeBase: do not claim Git protects the KB, require redundant backup trees, or silently relax KB state hashes, references, revision checks, preview, and failure reporting. This adaptation does not authorize unrelated experiment execution or external actions.
...
Use an existing Python 3.11+ runtime with `-X utf8 -B`; PDF preparation additionally needs optional `pypdf`. Check availability, never infer installation permission. Read relevant reference sections before unfamiliar operations. The blank core is orientation, not an experiment model: populate it from the owner assignment and verified in-scope evidence.
...
## Activation and host check

The root `AGENTS.md` instructs explicit loading; linked files are not guaranteed to load automatically on every host. In a fresh conversation, ask the agent to load the installed instructions and report what it actually read, dormant/bound status, assigned root if any and KB permission. Give no experiment assignment for that check: readiness requires no experiment scan or knowledge write. This tests loading and activation restraint separately from installation verification.

An owner can bind and enable routine KB use together:

> You are Seed Analyzer. Bind to the exact experiment root `D:\Experiments\Example`. Use `D:\Experiments\Example\KnowledgeBase` as working knowledge under the packaged KB policy. I authorize its bundled tools for retrieval, validation, registered source preparation and routine transactions, reading permitted sources in that experiment root and writing only derived knowledge inside that exact KnowledgeBase. Keep original sources read-only and non-executing. Inspect the folder, reconstruct the experiment, record material findings with provenance, report the Genesis Assessment and stop before experiment bootstrap or trial execution.

This is an example, not an issued instruction. Replace the path with the physical installation root, supply applicable exclusions/blind boundaries and narrow effects as needed. For discovery without KB execution/writes, explicitly withhold those effects. A generic empty state does not complete discovery.
...
| `KnowledgeBase/records.jsonl` | Typed records; initially empty. |
| `KnowledgeBase/sources.json` | Source catalog/revision; initially empty. |
...
## Intentional source revisions and Git
...
For preview continuation, keep the revision and transaction bytes fixed. First-page output includes `transaction_sha256`, `preview_time`, `next_offset`. Reissue identical preview arguments with `--offset NEXT --expect-transaction-sha256 HASH --preview-time TIME`; preserve limit/projection/budget choices. Inspect all relevant changes before apply. Preview time is prospective; actual apply generates current timestamps. Preview fields paginate without full values. Hashes establish identity/consistency, not approval.
